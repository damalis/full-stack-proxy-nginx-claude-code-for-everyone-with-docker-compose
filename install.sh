...
echo "==========================================================================="
echo "|                                                                         |"
echo "|   full-stack-proxy-nginx-claude-code-for-everyone-with-docker-compose   |"
echo "|                           by Erdal ALTIN                                |"
echo "|                                                                         |"
echo "==========================================================================="
sleep 2
...
anthropic_api_key=""
read -p 'Leave it blank for free or Enter Anthropic Api Key(https://platform.claude.com/login?returnTo=%2Fsettings%2Fkeys): ' anthropic_api_key
echo "Ok."
...
then
	...
	sudo -- sh -c -e "grep -qxF '127.0.1.1  claude.$domain_name' /etc/hosts || echo '127.0.1.1  claude.$domain_name' >> /etc/hosts"
else
	domain_name=""
...
then
	...
	./mkcert -uninstall && ./mkcert -install && ./mkcert -key-file privkey.pem -cert-file chain.pem $domain_name *.$domain_name && sudo cat privkey.pem chain.pem > fullchain.pem && sudo mkdir -p ../certbot/live/claude.$domain_name && sudo mv *.pem ../certbot/live/claude.$domain_name
	cd ..
	echo "Ok."
else
...
if [ -x "$(command -v docker)" ] && [ "$(docker compose version)" ]; then
	...
	# installing claude code and the other services
	docker compose up -d & export pid=$!
	echo "claude code and the other services installing proceeding..."
	echo
	wait $pid
	if [ $? -eq 0 ]
	then
		...
		if [ $? -ne 0 ]; then
			...
			echo
			until [ -n "$(sudo find ./certbot/live -name 'claude.$domain_name' 2>/dev/null | head -1)" ]; do
				echo "waiting Let's Encrypt certificates for claude.$domain_name"
				sleep 5s & wait ${!}
				if sudo [ -d "./certbot/live/claude.$domain_name" ]; then break; fi
			done
			echo "Ok."
		...
		echo
		echo "Error! could not installed claude code and the other services with docker compose" >&2
		echo
		exit 1
	fi
else
